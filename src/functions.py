...
class Function():
    ...
    def __init__(self, f, instance, domain, dimension):
        self.f = eval(str(f) + "(" + str(instance) + ")")
        self.domain = domain
        self.dimension = dimension
        self.f(np.random.random(dimension))
        
    def __call__(self, x):
        #x = self.normalize_x(x)
        #y = self.normalize_y(self.f(x))
        return self.f(x)

    def normalize_x(self, x):
        return x/self.domain[-1]
